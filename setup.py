...
class CMakeBuild(build_ext):
    def build_extensions(self):
        ...
        for ext in self.extensions:
            ...
            # 根据平台确定原始文件名
            if platform.system() == "Darwin":
                original_name = "libmagent.dylib"
            elif platform.system() == "Linux":
                original_name = "libmagent.so"
            elif platform.system() == "Windows":
                original_name = "magent.dll"
            ...
            # 执行重命名
            original_path = os.path.join(extdir, original_name)
            target_path = os.path.join(extdir, target_name)
            
            if os.path.exists(original_path):
                print(f"Renaming {original_path} to {target_path}")
                if os.path.exists(target_path):
                    os.remove(target_path)
                os.rename(original_path, target_path)
            else:
                print(f"Warning: Original library not found at {original_path}")
